...
openai.api_key = st.secrets.get("OPENAI_KEY", os.getenv("OPENAI_API_KEY"))
...
if USE_CREWAI:
    try:
        ...
        st.warning("CrewAI not available. Install with: pip install crewai>=0.86.0 langchain-openai>=0.3.0")
else:
    CREWAI_AVAILABLE = False
...
def header():
    ...
    if CREWAI_AVAILABLE:
        st.success("CrewAI Multi-Agent Mode Enabled")
...
if __name__ == "__main__":
    ...
    if lines:
        ...
        # CrewAI mode selection
        if CREWAI_AVAILABLE:
            inference_mode = st.radio(
                "Select Inference Mode:",
                options=["Standard GPT", "CrewAI (Simple)", "CrewAI (Full Quality Validation)"],
                index=0,
                help="CrewAI modes use multiple specialized agents for better quality answers"
            )
        else:
            inference_mode = "Standard GPT"
        ...
        if st.button("GO!"):
            ...
            if inference_mode == "CrewAI (Simple)" and CREWAI_AVAILABLE:
                with st.spinner("CrewAI agents are analyzing your query..."):
                    res = simple_crewai_inference(data, k=k_val, temperature=temp, api_key=openai.api_key)
            elif inference_mode == "CrewAI (Full Quality Validation)" and CREWAI_AVAILABLE:
                with st.spinner("CrewAI agents are analyzing your query with full validation..."):
                    res = crewai_inference(data, k=k_val, temperature=temp, api_key=openai.api_key)
            else:
